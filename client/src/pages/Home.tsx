...
export default function Home() {
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-50">
              ...
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[0.85] text-balance">
                {homeContent ? (
                  homeContent.heroTitle.split(" ").map((word, i) => (
                    <span key={i} className={i % 2 === 1 ? "text-primary bg-accent px-2 rounded-lg" : ""}>{word} </span>
                  ))
                ) : (
      ...
      <section id="transparencia" className="py-24 bg-slate-900 text-white overflow-hidden relative">
        <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-3xl" />
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full border border-white/10 text-xs font-bold uppercase tracking-widest mb-6">
                <span className="w-2 h-2 rounded-full bg-accent animate-pulse" />
                Rendición de Cuentas
              </div>
              <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6">
                Transparencia y <span className="text-accent">Rendición</span>
              </h2>
            ...
            <div className="relative">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-8 shadow-2xl">
                <div className="flex items-center justify-between mb-8">
                  <div className="font-bold text-xl">Actividad Reciente</div>
                  <div className="px-3 py-1 bg-accent/20 text-accent rounded-full text-xs font-bold">En Tiempo Real</div>
                </div>
                <div className="space-y-6">
                  {activities?.slice(0, 2).map((activity) => (
                    <div key={activity.id} className="flex gap-4 p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors border border-white/5">
                      ...
                      <div>
                        ...
                        <div className="font-bold text-white line-clamp-1">{activity.title}</div>
                        <div className="text-xs text-white/60 line-clamp-1">{activity.description}</div>
                      </div>
                    </div>
                  ))}
                  <Button variant="outline" className="w-full border-white/20 text-white hover:bg-white/10" onClick={() => document.getElementById("actividades")?.scrollIntoView({behavior: 'smooth'})}>
